  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      ...
      <div className="sticky top-4 z-40 px-4 pb-4 sm:px-6 lg:px-8 pointer-events-none">
        <header className="pointer-events-auto mx-auto flex w-full max-w-[1600px] flex-col lg:flex-row lg:items-center lg:justify-between gap-4 rounded-[2rem] lg:rounded-full border border-white/20 bg-gradient-to-br from-cyan-500/10 via-slate-900/95 to-fuchsia-500/10 px-6 py-4 lg:px-8 shadow-[0_8px_32px_-8px_rgba(34,211,238,0.2)] backdrop-blur-xl transition-all">
          
          <div className="flex items-center justify-between gap-4 lg:w-auto">
            <Link to="/" className="flex items-center gap-3">
              <div className="flex h-10 w-10 lg:h-11 lg:w-11 items-center justify-center rounded-2xl border border-cyan-400/30 bg-cyan-400/10 text-xl text-cyan-200 shadow-neon">
                ⚡
              </div>
              <div>
                <p className="text-xl lg:text-2xl font-bold tracking-tight text-white leading-none">MemeSense <span className="text-cyan-300">AI</span></p>
                <p className="text-xs text-slate-400 hidden sm:block">Meme coin intelligence platform</p>
              </div>
            </Link>
          ...
          <nav className="hidden lg:flex items-center gap-1 xl:gap-2">
            {navItems.map((item) => (
              <NavLink
                ...
                to={item.to}
                className={({ isActive }) =>
                  `rounded-full px-3 py-2 text-sm xl:px-4 transition whitespace-nowrap ${
                    isActive
          ...
          <div className="hidden lg:flex items-center gap-3">
            <span className={`rounded-full border px-3 py-2 text-sm whitespace-nowrap ${liveTone}`}>
              {usingMockSocialData ? 'Fallback signals' : 'Live social data'}
            </span>
            <span className="hidden xl:inline-block rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-2 text-sm text-cyan-200 whitespace-nowrap">
